    ...
    for i in range(0, len(razrez)):  # идём по списку
        ...
        if (razrez[i].count("x") == 1):  # Если в этом куске многочлена есть x
            ...
            if (razrez[i].count("^") == 0):  # если x в первой степент
                ...
                razrez[i].append(str(step + 1))  # Закидываем степень +1 т.к. у нас уже 1ая
                ...
                znach = int(bufStr)  # перекидываем из строки в int
                znach = znach + step  # прибавляем нужную степень
                bufStr = str(znach)  # перекидываем из int в str
